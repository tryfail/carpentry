#!/bin/ksh
# the purpose of this script is to check if there any changes in the external carp (usually carp0)
...
ifconfig_local_rc=$?
ifconfig_remote=$(${ssh_cmd} "$remote" "ifconfig carp | grep -E 'status: invalid|carp: INVALID'")
ifconfig_remote_rc=$?
